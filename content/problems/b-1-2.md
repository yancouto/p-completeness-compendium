...
Open (Dessmark, Lingas, and Maheshwari [1]).

## Remarks

This problem was proved to be in $\P$ in [2]. They give a $\NC$ approximation algorithm for the problem that finds an edge ranking within a factor of two of optimal. An $\NC$ algorithm for constant degree trees is also given in [2]. A similar problem called the *Node Ranking Problem* for trees in which the vertices are labeled, instead of the edges, was proved to be in $\NC$ (de la Torre and Greenlaw [3], de la Torre, Greenlaw, and Przytycka [4]). See [FILL]({{< relref "./a-8-25.md" >}}) for a related $\P$-complete problem. Karloff and Shmoys give $\NC$ algorithms for several versions of edge ranking multigraphs [5], a problem having a similar flavor. See [LFDVC]({{< relref "./a-2-6.md" >}}) and [EC]({{< relref "./b-9-3.md" >}}) for a description of their results.
