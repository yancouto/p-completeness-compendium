...
This problem is stated as a computation problem. Thus, it is technically not in $\P$. The reduction to show hardness is from [CVP]({{< relref "./a-1-1.md" >}}). Given $\alpha$ it is easy to construct a deeper circuit such that if we could approximate the output of the new circuit, we could determine the output of $\alpha$.
