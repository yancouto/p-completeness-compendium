...
The parallel complexity of *Different Than Majority Labeling* (DTML) was posed as an open question by Luby [1]. DTML was also discussed by Godbeer [2] and the problem was reduced to a version of a connectionist model problem, see SCCM. Schäffer and Yannakakis proved the problem to be equivalent to UMCS [3]. Therefore, the problem is $\P$-complete. Sarnath and He show that LFDTML remains $\P$-complete when restricted to planar graphs [4]. They also give another sequential algorithm for the problem and state that a decision problem based on this algorithm is also $\P$-complete [4].