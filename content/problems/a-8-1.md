...
$\P$-complete (Papadimitriou and Tsitsiklis <a href="#ref-2" class="reference-citation">[2]</a>).

## Remarks

There is a polynomial time algorithm for the problem that uses dynamic programming (Howard <a href="#ref-3" class="reference-citation">[3]</a>). The reduction to show hardness is from [MCVP]({{< relref "./a-1-3.md" >}}).

The reduction shows that the finite horizon stationary version of the problem is $\P$-hard. This problem is not known to be in $\P$. The *deterministic* version of the FHMDP, which requires that $p$ only has values $0$ or $1$, is in $\NC$ <a href="#ref-2" class="reference-citation">[2]</a>. Note, this last result holds for both the stationary and nonstationary versions of the problem.
