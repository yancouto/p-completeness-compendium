---
title: "Monadic Krom Satisfiability"
acronym: "MKS"
book_id: "A.6.4"
categories: ["Logic"]
status: "p-complete"
tags: []
references: [89, 88]
related_problems:
  - id: a-7-8
    relation: see-also
---

## Given

A monadic Krom formula $F$.

A *Krom formula* is a $2$-CNF formula. That is, a formula in CNF with at most two disjuncts per clause. Let $\varphi$ and $\psi$ be predicate letters that may be preceded by a negation. A conjunct of $F$ is *monadic* if it is of the form $(\varphi y_1 y_1 \vee \psi y_1 y_1)$.

## Problem

Is $F$ satisfiable?

## Status

$\P$-complete (Denenberg and Lewis <a href="#ref-1" class="reference-citation">[1]</a>).

## Remarks

The reduction to show hardness is from a Nonacceptance Problem for Pushdown Automata (Denenberg <a href="#ref-2" class="reference-citation">[2]</a><a href="#ref-1" class="reference-citation">[1]</a>), see [2DPDA]({{< relref "./a-7-8.md" >}}).
